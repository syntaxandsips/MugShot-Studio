
'use client';

import { useState } from 'react';
import { useForm } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { z } from 'zod';
import { Step1Schema, Step2Schema, Step3Schema } from '@/src/schemas/auth.schema';
import { WizardProgressBar } from './WizardProgressBar';
import { Step1Basics } from './steps/Step1Basics';
import { Step2Identity } from './steps/Step2Identity';
import { Step3Verification } from './steps/Step3Verification';
import { Step4Success } from './steps/Step4Success';
import Link from 'next/link';

const FullSchema = Step1Schema.and(Step2Schema).and(Step3Schema);

const WIZARD_STEPS = [
    { id: 1, label: 'Account' },
    { id: 2, label: 'Identity' },
    { id: 3, label: 'Details' },
    { id: 4, label: 'Verify' },
];

export function SignupWizard() {
    const [step, setStep] = useState(1);

    const form = useForm<z.infer<typeof FullSchema>>({
        resolver: zodResolver(FullSchema),
        mode: 'onChange',
        defaultValues: {
            newsletterOptIn: false,
            referralCode: '',
        }
    });

    const email = form.getValues('email');

    return (
        <div className="space-y-5">
            <div className="space-y-1">
                <h2 className="text-2xl font-bold text-gray-900">Create an account</h2>
                <p className="text-gray-500 text-sm">
                    Join MugShot Studio and start creating today.
                </p>
            </div>

            <WizardProgressBar
                currentStep={step}
                totalSteps={WIZARD_STEPS.length}
                steps={WIZARD_STEPS}
            />

            <div className="space-y-4">
                {step === 1 && <Step1Basics form={form} onNext={() => setStep(2)} />}
                {step === 2 && <Step2Identity form={form} onNext={() => setStep(3)} onBack={() => setStep(1)} />}
                {step === 3 && <Step3Verification form={form} onComplete={() => setStep(4)} onBack={() => setStep(2)} />}
                {step === 4 && <Step4Success email={email} />}
            </div>

            {step < 4 && (
                <div className="space-y-5">
                    {/* Social login divider */}
                    <div className="relative">
                        <div className="absolute inset-0 flex items-center">
                            <span className="w-full border-t border-gray-200" />
                        </div>
                        <div className="relative flex justify-center text-[10px] uppercase font-bold tracking-widest">
                            <span className="bg-gray-50 px-2 text-gray-400">or continue with</span>
                        </div>
                    </div>

                    <div className="grid grid-cols-2 gap-3">
                        {/* Google Button */}
                        <button className="h-10 bg-white border border-gray-200 hover:bg-gray-50 rounded-full flex items-center justify-center transition-colors shadow-sm">
                            <svg className="w-4 h-4 mr-2" viewBox="0 0 24 24">
                                <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
                                <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
                                <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
                                <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
                            </svg>
                            <span className="text-xs font-bold text-gray-700">Google</span>
                        </button>

                        {/* Apple Button */}
                        <button className="h-10 bg-black hover:bg-black/90 rounded-full flex items-center justify-center transition-colors shadow-sm">
                            <svg className="w-4 h-4 mr-2 text-white" fill="currentColor" viewBox="0 0 24 24">
                                <path d="M17.05 20.28c-.96.95-2.06 1.92-3.37 1.92-1.27 0-1.61-.74-3.14-.74-1.55 0-1.94.72-3.14.74-1.25.02-2.32-.94-3.37-1.92-2.14-2-3.77-5.63-3.77-8.81 0-5.18 3.38-7.91 6.57-7.91 1.68 0 3.12 1.05 4.04 1.05.9 0 2.62-1.22 4.54-1.02.82.04 3.1.33 4.56 2.45-.12.07-2.73 1.58-2.73 4.7 0 3.73 3.26 4.95 3.3 4.97-.03.07-.52 1.78-1.72 3.57zM13.71 4.2c-.82 1.02-1.98 1.76-3.08 1.76-.14 0-.27-.01-.39-.03.1-.96.65-2.02 1.34-2.82 1.14-1.32 2.37-2.13 3.33-2.13.14 0 .28.01.41.03-.1.97-.73 2.11-1.61 3.19z" />
                            </svg>
                            <span className="text-xs font-bold text-white">Apple</span>
                        </button>
                    </div>

                    <div className="text-center text-xs">
                        <span className="text-gray-400">Already have an account? </span>
                        <Link href="/auth/signin" className="font-bold text-[#0f7d70] hover:underline">
                            Sign in
                        </Link>
                    </div>
                </div>
            )}
        </div>
    );
}
